...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None, sug_action=None):
        ...
        logits = self.actor(features) # actor网络输出动作的logits值
        probs = Categorical(logits=logits) # # 创建类别分布

        if action is None:
            action = probs.sample()

        value = self.critic(x) # 使用critic网络评估状态价值

        sug_logprob = None
        if sug_action is not None:
            valid_mask = (sug_action >= 0)
            if valid_mask.any(): 
                sug_action_clamped = torch.clamp(sug_action, min=0) # # 限制建议动作的范围
                sug_logprob = probs.log_prob(sug_action_clamped) # 计算建议动作的对数概率
            else:
                sug_logprob = None
...
if __name__ == "__main__":
    ...
    max_avg_tons = 0
    avg_tons = 0
    for iteration in range(start_iteration + 1, args.num_iterations + 1): # 开始回合训练迭代
        ...
        for step in range(args.num_steps): # 每一步的决策
        ...
        clipfracs = []
        for epoch in range(args.update_epochs): # 采集完一回合的数据后开始分多轮更新模型参数
