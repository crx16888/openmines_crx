...
# 状态空间比较复杂，有事件类型信息 ：[1,0,0] 表示init进程，[0,1,0] 表示haul，[0,0,1] 表示unload，卡车状态信息包括位置量等，
# 目标状态特征如队列长度和容量等、道路状态信息...
# 动作空间有三种不同的维度，如init事件动作维度：load_num表示：选择初始装载点
# 反映了每个状态下矿卡选择目标点的概率分布
...
class ActorCritic(nn.Module):
    ...
    def act(self, state, event_type, action=None): # 根据当前状态选择动作
    ...
    def evaluate(self, state, action, event_type): # 评估状态动作价值
        encoded_state = self.actor_encoder(state)
        if event_type.dim() > 0:
